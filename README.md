...
jobs:
  build:
    ...
    steps:
      ...
      - name: Run JSON Lint
        run: bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/json-lint/master/src/pipeline.sh)
```
...
```bash
bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/json-lint/master/src/pipeline.sh)
```
